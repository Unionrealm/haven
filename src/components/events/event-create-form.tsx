...
export function EventCreateForm() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      location: '',
      category: '',
      ticket_types: [{ name: '일반 티켓', description: '', price: 0, quantity: 100 }],
    },
  })
  ...
  const validateStep = async (stepNum: number) => {
    ...
    if (stepNum === 1) fields = ['title', 'description', 'date', 'time', 'category']
    if (stepNum === 2) fields = ['venue', 'address', 'location']
    if (stepNum === 3) fields = ['ticket_types']
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const dateTime = `${data.date}T${data.time}:00+09:00`

      // Mock submission - in production this would hit Supabase
            ...
            <button
              ...
              className={cn(
                ...
                step === s.id
                  ...
                  : step > s.id
                    ? 'text-[var(--foreground)] cursor-pointer hover:bg-[var(--muted)]'
        ...
        <Card>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div
                className="relative aspect-[16/7] rounded-xl border-2 border-dashed border-[var(--border)] overflow-hidden cursor-pointer hover:border-[var(--foreground)] transition-colors group"
                onClick={() => {
              ...
              >
                {previewImage ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-[var(--muted-foreground)]">
                    ...
                    <span className="text-xs">클릭하여 URL 입력</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium">이벤트 소개 *</label>
              <textarea
                placeholder="이벤트에 대해 소개해주세요. 공연 정보, 주의사항 등을 자유롭게 작성하세요."
                className={cn(
                  'w-full min-h-[160px] rounded-lg border border-[var(--border)] bg-transparent px-3 py-2 text-sm',
        ...
        <Card>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="rounded-lg bg-[var(--muted)] p-4 text-sm text-[var(--muted-foreground)]">
              <p className="font-medium text-[var(--foreground)] mb-1">카카오맵 연동</p>
              <p>주소를 입력하면 이벤트 페이지에 카카오맵이 자동으로 표시됩니다.</p>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <div className="space-y-4">
            ...
            <Card key={field.id}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-medium text-sm">티켓 {index + 1}</h3>
                  {fields.length > 1 && (
                    <button
                      type="button"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          step="1000"
                          placeholder="0"
                          className="w-full h-10 pl-7 pr-3 rounded-lg border border-[var(--border)] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-[var(--ring)] transition-colors"
                          {...form.register(`ticket_types.${index}.price`, { valueAsNumber: true })}
                        />
                      </div>
                      <p className="text-xs text-[var(--muted-foreground)]">0원 = 무료 행사</p>
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        min="1"
                        placeholder="100"
                        className="w-full h-10 px-3 rounded-lg border border-[var(--border)] bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-[var(--ring)] transition-colors"
          ...
          <div className="rounded-lg border border-[var(--border)] p-4 text-sm">
            ...
            <p className="text-[var(--muted-foreground)]">
              유료 티켓의 경우 판매 금액의 <strong className="text-[var(--foreground)]">3%</strong>가 서비스 수수료로 공제됩니다.
              무료 이벤트는 수수료가 없습니다.
            </p>
          </div>
        </div>
        ...
        {step < STEPS.length ? (
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              onClick={() => form.setValue('cover_image', form.getValues('cover_image') || '')}
            >
              임시저장
            </Button>
